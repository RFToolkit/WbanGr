...
cfg_pcap_file = './perso/wpan7.pcap'
cfg_dictionary_file = './perso/7-more-passwords.txt'
...
def get_challenge_response():
    ...
    lens = map(lambda x: len(x), r)
    pckt_lens = dict(enumerate([l for l in lens], 1))
    ...
    _t1 = dict(enumerate([l for l in [*pckt_lens.keys()][pckt_108:]], 1))
    ...
    for pckt_indx in sorted(_t1.keys()):
        pckt_len = _t1[pckt_indx]
        try:
            test=tcp(r[pckt_indx].load.hex(), '7202000f32')[0].encode('utf-8', 'ignore').hex()
            if pckt_len+14 == 84 and hexlify(test)[14:24] == '7202000f32':
                pckt_84 = pckt_indx
                break
            if pckt_len+14 == 92 and hexlify(test)[14:24] == '7202001732':
                pckt_92 = pckt_indx
                break
        except:
            pass
